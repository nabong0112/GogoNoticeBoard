		...
		<div align="center" style="border: 1px solid #dddddd; overflow: auto;">
			...
			<table class="table table-striped"
				...
				<thead>
					<tr>
						<th colspan="7"
							style="background-color: #eeeeee; text-align: center;"><%=board_no%>번째
							글</th>
					</tr>
				</thead>
				<tbody>
			...
			<!-- ----------------------------------------------- 글 보여주는곳 끝 --------------------------------------------------------- -->
			<div align="center">
				<a href="noticeboard.jsp" class="btn btn-primary">목록으로</a>
				<%
					//글작성자 본인일시 수정 삭제 가능 
					if (user_id != null) {
				...
				<a href="/Nabong_writer/UpdateServlet?board_no=<%=board_no%>"
					class="btn btn-primary">수정하기</a> <a
			...
			<!-- --------------------------------------------------댓글 보여주는곳---------------------------------------------------------- -->
			<script type="text/javascript">
				var bDisplay = true; //답글 안보였다가 누르면 보이고 또누르면 안보임
				function doDisplay() {
					var con = document.getElementById("recomment");
					if (con.style.display == 'none') {
						con.style.display = 'line-block';
					} else {
			...
			<div id="comment-border" style="align-content: center; border: 1px solid #dddddd;">
				...
				<form method="post" action="/Nabong_writer/CommentServlet?board_no=<%=board_no%>" style="margin: 0">
				<table
					style="text-align: center; border: 1px solid #dddddd; line-height: 2.3em;">
					<tr align="right">
					...
					<tr align="center" id = "recomment" style="display: none;"> 
					<td><%= user_id %></td>
